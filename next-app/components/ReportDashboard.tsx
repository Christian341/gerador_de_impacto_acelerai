...
export const ReportDashboard: React.FC<ReportDashboardProps> = ({ result, sourceImage, sourceText }) => {
    ...
    return (
        <div className="w-full space-y-6 md:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-4 md:gap-6">
                <div className="md:col-span-8 glass-panel p-6 md:p-8 rounded-2xl flex flex-col md:flex-row items-center gap-6 md:gap-10 border-white/5 relative overflow-hidden">
                    <div className="relative flex-shrink-0 flex items-center justify-center w-[120px] h-[120px]">
                        ...
                        <div className="absolute inset-0 flex flex-col items-center justify-center">
                            <span className={`text-3xl md:text-4xl font-black ${getScoreColor(result.overall_score)} leading-none`}>{result.overall_score}</span>
                            <span className="text-[8px] md:text-[9px] text-slate-500 font-black uppercase tracking-[0.2em] mt-1">Impacto</span>
                        </div>
                    </div>
                ...
                <div className="md:col-span-4 glass-panel p-6 md:p-8 rounded-2xl flex flex-row md:flex-col items-center justify-center text-center border-white/5 gap-4">
                    ...
                    <div>
                        <h3 className="text-lg md:text-xl font-black text-white tracking-tight leading-none">Tom de Voz</h3>
                        <p className="text-[8px] md:text-[10px] text-slate-500 mt-1 md:mt-2 uppercase tracking-[0.3em] font-black">{getSentimentLabel(result.sentiment)}</p>
                    </div>
                </div>
            </div>
            ...
            <div className={`grid grid-cols-1 ${isDocument ? 'lg:grid-cols-1' : 'lg:grid-cols-12'} gap-6 md:gap-8`}>
                ...
                <div className={isDocument ? 'w-full grid grid-cols-1 lg:grid-cols-2 gap-8' : 'lg:col-span-7 space-y-8'}>
                    <section className="space-y-6">
                        <h3 className="text-[10px] font-black text-purple-500 uppercase tracking-widest flex items-center gap-3">
                            ...
                            {result.agents_feedback?.map((agent, idx) => (
                                <div key={idx} className="glass-panel p-5 md:p-6 rounded-xl border-white/5 hover:border-white/10 transition-all">
                                    <div className="flex justify-between items-center mb-3">
                                        <h4 className="text-base font-black text-white">{agent.agent_name}</h4>
                    ...
                    <section className="space-y-6">
                        <div className="glass-panel p-6 md:p-8 rounded-3xl border-blue-500/10 shadow-2xl bg-[#0f0f11]">
                            <h3 className="text-lg font-black text-white mb-6">Impacto em Clientes Mapeados</h3>
                            ...
                            <div className="mt-8 pt-8 border-t border-white/5">
                                <h4 className="text-[10px] font-black text-blue-500 uppercase tracking-widest mb-4">Diretrizes de Otimização</h4>
